    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4 items-start">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs text-[var(--text-dim)] uppercase tracking-wider mb-1">{playlist.is_public ? 'Публичный плейлист' : 'Приватный плейлист'}</p>
          {editing ? (
            <div className="space-y-2 mb-3">
              ...
              <div className="flex gap-2">
                <button onClick={saveEdit} className="px-3 py-1.5 bg-[var(--accent)] text-white rounded-lg text-sm">Сохранить</button>
                <button onClick={() => setEditing(false)} className="px-3 py-1.5 border border-[var(--border)] text-[var(--text-dim)] rounded-lg text-sm">Отмена</button>
              </div>
            </div>
          ) : (
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {playlist.tracks && playlist.tracks.length > 0 && (
              <button onClick={playAll} className="px-5 py-2.5 bg-[var(--accent)] text-white rounded-full text-sm font-medium hover:opacity-90 transition">▶ Играть все</button>
            )}
            {isOwner && !editing && (
              <>
        ...
        <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] overflow-hidden">
          {playlist.tracks.map((track, idx) => (
            <div key={track.id} className="flex items-center">
              ...
              {isOwner && (
                <Tooltip text="Убрать из плейлиста">
