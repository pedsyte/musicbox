    ...
    <div className={`relative ${className}`} ref={ref}>
      ...
      {open && formats.length > 0 && (
        <div className={`absolute ${compact ? 'right-0 bottom-8' : 'right-0 top-8'} w-52 bg-[var(--surface)] border border-[var(--border)] rounded-lg shadow-xl z-[60] py-1 overflow-hidden`}>
          <p className="px-3 py-1.5 text-[10px] text-[var(--text-dim)] uppercase tracking-wider">Формат скачивания</p>
          {formats.map(fmt => (
            <a
              key={fmt}
            ...
            >
              ...
              {fmt === originalFormat && (
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--accent)]/15 text-[var(--accent)]">оригинал</span>
              )}
            </a>
          ))}
        </div>
